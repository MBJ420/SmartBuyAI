...
async function startSearch() {
    ...
    try {
        ...
        const response = await fetch(`${BACKEND_URL}/search?q=${encodeURIComponent(searchInput)}`);
...
async function fetchFinalData(query) {
    try {
        // Since status is success, calling /search again will instantly return the cached data
    ...
    if (!products || products.length === 0) {
        resultsGrid.innerHTML = `
            <div class="col-12 text-center mt-4">
                <h4 class="text-muted">No products found. Try a different search!</h4>
            </div>
        `;
        return;
    }
            ...
            <div class="col-md-6 col-lg-4 mb-4">
                <div class="card product-card h-100">
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <p class="product-price mt-auto mb-3">Rs. ${formattedPrice}</p>
                        <a href="${product.link}" target="_blank" class="btn btn-outline-primary w-100 mt-auto">
